	...
	for l in range(pix): #loop che spara raggi riga per riga
		...
		for f in range(pix/ncpus): #loop che spara raggi colonna per colonna
			...
			for i in range(1,N):
				...
				if x_sfer[1]>70.0: #ATTENZIONE dipendente dalla dimensione minima del box. Solo per risparmiare tempo
					x, y, z=stoc(x_sfer[1], x_sfer[2], x_sfer[3])
		
					if (y>=y_box*0.5) or (y<=-y_box*0.5) or (z>=z_box*0.5) or (z<=-z_box*0.5):
						jesoo[l,f,:] = numpy.ones((3), numpy.uint8)*255
						break
...
##valori iniziali
m=8. #input("Che massa vuoi dare al buco nero?\n")
h=1.
N=300 #max iterations
hlim=array([0.01, 5])
errlim=([1e-10, 1e-5])
pix=128 #input("Quanti pixel vuoi fare per questa immagine?\n") #minimo 8 come le cpu

largh_image=2.
alt_image=2.

#for large images
#x_in=-600. #distanza dal buco nero lungo x del film
